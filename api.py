...
from database_manager import db_manager, AccountEntity, PeerEntity, TokenEntity, TagEntity, AddressBookEntity
...
TOKEN_LIFETIME_MS = 24 * 3600 * 1000 # 令牌存活24小时

class UUIDUtils:
    @staticmethod
    def string(include_dash: bool = False) -> str:
...
def add_user():
    ...
    with db_manager.new_session() as session:
        ...
        session.add(AccountEntity(id=UUIDUtils.string(), account=add_req.get('account'), password=add_req.get('password'), create_at=create_at))
        session.commit()
...
def heartbeat():
    ...
    with db_manager.new_session() as session:
        peer = session.query(PeerEntity).filter_by(client=heartbeat_req.get('id')).first()
        if peer:
            peer.uuid = heartbeat_req.get('uuid')
            peer.modified_at = int(datetime.now().timestamp() * 1000)

            login_token = session.query(TokenEntity).filter_by(peer_id=peer.id).first()
            if login_token:
                login_token.expire_at = peer.modified_at + TOKEN_LIFETIME_MS #  自动保活设备登录的账号
...
def sysinfo():
    ...
    with db_manager.new_session() as session:
        peer = session.query(PeerEntity).filter_by(client=sysinfo_req.get('id')).first()
        if not peer:
            peer = PeerEntity(id=UUIDUtils.string(), client=sysinfo_req.get('id'))
            session.add(peer)

        peer.os = sysinfo_req.get('os')
        peer.cpu = sysinfo_req.get('cpu')
        peer.uuid = sysinfo_req.get('uuid')
        peer.memory = sysinfo_req.get('memory')
        peer.version = sysinfo_req.get('version')
        peer.hostname = sysinfo_req.get('hostname')
        peer.username = sysinfo_req.get('username')
        peer.modified_at = int(datetime.now().timestamp() * 1000)
        session.commit()
...
def login():
    ...
    with db_manager.new_session() as session:
        ...
        for login_token in session.query(TokenEntity).filter_by(account_id=account.id).all():
            ...
            if login_token.peer.client == login_req.get('id'): # 同一设备已登录直接返回token
        ...
        # 保存登录设备信息
        peer = session.query(PeerEntity).filter_by(client=login_req.get('id')).first()
        if not peer:
            peer = PeerEntity(id=UUIDUtils.string(), client=login_req.get('id'))
            session.add(peer)

        peer.uuid = login_req.get('uuid')
        peer.hostname = login_req.get('deviceInfo').get('name')
        peer.modified_at = int(datetime.now().timestamp() * 1000)

        # 生成新的用户令牌
        if not login_access_token:
            login_access_token = UUIDUtils.string()
            login_token = TokenEntity(id=login_access_token, account_id=account.id, peer_id=peer.id)
...
def update_address_book():
    ...
    with db_manager.new_session() as session:
        ...
        if not tag:
            tag = TagEntity(id=UUIDUtils.string(), account_id=login_token.get('account_id'))
            session.add(tag)
        ...
        for address_book in address_books.get('peers', []):
            peer = session.query(PeerEntity).filter_by(client=address_book.get('id')).first()
            if not peer:
                peer = PeerEntity(id=UUIDUtils.string(), uuid=address_book.get('id'), client=address_book.get('id'))
                session.add(peer)

            peer.username = address_book.get('username')
            peer.hostname = address_book.get('hostname')
            peer.platform = address_book.get('platform')
            peer.modified_at = int(datetime.now().timestamp() * 1000)

            session.add(AddressBookEntity(id=UUIDUtils.string(), account_id=login_token.get('account_id'), peer_id=peer.id
                        , tags=','.join(address_book.get('tags')), alias=address_book.get('alias'), hash=address_book.get('hash')))
        session.commit()
...
def get_address_book():
    ...
    with db_manager.new_session() as session:
        ...
        for address_book in session.query(AddressBookEntity).filter_by(account_id=login_token.get('account_id')).all():
            peer = {'id': address_book.peer.client}
            peer['username'] = address_book.peer.username
            peer['hostname'] = address_book.peer.hostname
            peer['platform'] = address_book.peer.platform
